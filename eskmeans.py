...
def process_embeddings(embedding_mats, vec_ids_dict):
    ...
    return (np.asarray(embeddings, dtype=float), vec_ids, ids_to_utterance_labels)
...
def main():
    ...
    # Vector IDs
    n_slices = 3
    vec_ids1 = -1 * np.ones((n_slices**2 + n_slices)//2, dtype=int)
    vec_ids2 = -1 * np.ones((n_slices**2 + n_slices)//2, dtype=int)
    i_embed = 0
    n_slices_max = 20
    for cur_start in range(n_slices):
        for cur_end in range(cur_start, min(n_slices, cur_start + n_slices_max)):
            ...
            t = cur_end_
            i = t * (t - 1) // 2
            vec_ids1[i + cur_start] = i_embed
            vec_ids2[i + cur_start] = i_embed
            i_embed += 1
